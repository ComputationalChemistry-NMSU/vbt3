...
Valence Bond Theory is the chemist's intuitive language for electronic structure: each Lewis resonance structure is a concrete picture of electron pairing, and the full wavefunction is a linear combination of such structures. Despite this intuitive appeal, quantitative QVBT calculations are hampered by the non-orthogonality of the underlying Slater determinants — every matrix element involves overlap cofactors that couple all pairs, and hand derivations explode combinatorially once the system has more than a handful of electrons.

This paper addresses the bottleneck with a symbolic-computation approach. The key contributions are:

1. **A symbolic VB engine** (**vbt3**) that constructs all one- and two-electron matrix elements between non-orthogonal VB determinants in SymPy, using only the Slater–Condon rules and a small set of generators, and eliminates redundant arithmetic through a fast path for precomputed half-determinants.
2. **Automatic symmetry detection** that discovers the permutation group leaving a given (H, S) pair invariant and builds the symmetry-adapted basis automatically, via either degeneracy analysis of numerical eigenvectors or graph-automorphism search.
3. **Exact analytical results for benzene** in both the one-electron and the two-electron (Hubbard) regimes, including (a) a closed-form rational-function dependence on AO overlap `s` in the one-electron case, (b) a Rayleigh–Schrödinger perturbation series in `U` whose rational coefficients decode analytically from Hückel molecular-orbital combinatorics, and (c) a [2/4] Padé resummation that extends the domain of validity across the full physically relevant `U` range.

The approach applies uniformly to smaller reference systems (H₂ dimer, allyl anion) and scales through symmetry reduction to systems as large as the 400-determinant benzene full-CI space.
